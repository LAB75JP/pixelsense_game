...
public class CameraFlyInState : ApplicationState <CameraFlyInState> {
	...
	protected override void Enter () {
		m_SplineEnumerator.MoveToStart ();
	}
	...
	protected override void PerformUpdate () {
		if (m_Value01Timer.Value01 < 1.0f) {
			...
			m_SplineEnumerator.MoveTo (l_SmoothedValue * m_SplineEnumerator.Spline.LastControlPoint.T);
			...
			CameraManager.Instance.transform.rotation = Quaternion.Slerp (l_IntroductionRotation, l_PlayRotation, l_SmoothedValue);
			
		} else {
			if (!ApplicationStateManager.Instance.IsStateChanging) {
				ApplicationStateManager.Instance.TransitionToMode (PlayGameState.Instance, AirhockeyManager.Instance.guiTransitionDuration);
			}
		}
	}
